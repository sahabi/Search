...
def point_inspect(pre_ve,p,xc=None,xr=None,s=None,d_t=None):
    vs = get_vs(p,aoa(pre_ve,p,xc,xr,d_l),d_l = s)
    ve = get_ve(p,aoa(pre_ve,p,xc,xr,d_l),d_t)
    path_task1 = get_path(pre_ve, vs, turning_radius, step_size = .5)
    path_task2 = get_path(vs, ve, turning_radius, step_size = .5)
    return path_task1+path_task2

def line_search(pre_ve,line,angles,smoothing=None, b = False):
    if not b:
        angles = 0
    ...
    paths = map(get_path,seg_vs,seg_ve,[1]*len(line),[0.5]*len(line))
    path = [item for sublist in paths for item in sublist]
    return path

def spiral_search(p,d_w,d_l,alpha,extent, step = deg2rad(36)):
    ...
    vs = v[:-1]
    ve = v[1:]
    assert (len(vs)< 150), "The length of points %r is probably too high!" % len(vs)
    paths = map(get_path,vs,ve,[1]*len(vs),[5]*len(vs))
    path = [item for sublist in paths for item in sublist]
    return path
...
def area_search(pre_ve,poly,angle,alpha,d_w,p=None,xc=None,xr=None,s=None,d_t=0,d_l=0):
    
    y = []
    v = []
    
    if isinstance(p,tuple):
        path = point_inspect(pre_ve,p,xc,xr,s,d_t)
        pre_ve = path[-1]
    else:
        path = []
    ...
    paths = map(get_path,seg_vs,seg_ve,[1]*len(v),[5]*len(v))
    path = [item for sublist in paths for item in sublist]
    return path
